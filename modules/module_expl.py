...
def init(botconfig):
    """ bot initializatin """
    global config
    config=botconfig.get("module_expl", [{}])[0]
    print 10*"::",
    print config
...
def expl_getdir(channel):
    ...
    if not os.path.exists(expldir): 
        print "::::no directory"
        if config.get("autocreate"):
            print "::::: creating", expldir
            # bot.log("Creating expldir: %s" % expldir)
            os.makedirs(expldir)
        else:
            # bot.log("Not expldir: %s . Set 'autocreate' to true in configuration." % expldir)
            return None
    return expldir
...
def check_params(bot, args, channel):
    ...
    bot.log("check_params(%(bot)s, %(args)s, %(channel)s" % locals())
    if not args: 
        return False
...
def command_add(bot, user, channel, args):
    ...
    args = args.split(" ", 1)
    if not args[1]: 
        return bot.say(channel, "No explanation given")
  
    term = expl_parseterm(args[0])
    if term in termlist:
        return bot.say(channel, "Term '%s' already exists." % term)
    ...
    return bot.say(channel, "Term '%s' added: %s" % (term, expl))
...
def command_del(bot, user, channel, args):
    ...
    if not term in termlist:
        return bot.say(channel, "Term '%s' doesn't exist." % term)
 
    expl = expl_getexpl(expldir, term)
    os.unlink(os.path.join(expldir, term))
    bot.log("Term '%s' for %s deleted by %s (contained: %s)" % (term, channel, user, expl))
    return bot.say(channel, "Term '%s' deleted (contained: %s)" % (term, expl))
...
def command_ls(bot, user, channel, args):
    ...
    if matches == 0: return bot.say(channel, "No term matched '%s'." % pattern)
    first = ""
    if matches > 20: first = "first 20 of "
 
    matchlist.sort()
    return bot.say(channel, "Terms matching '%s' (%stotal %d): %s" % (pattern, first, matches, ", ".join(matchlist[0:20])))
